...
必须独立选择并逐项确认，不能按名称相似度自动映射。影响观测默认开启，仅在创建/复用
解决单时记录一次基线；风险等级优先级默认高风险 P1、中风险 P2、低风险 P3。这些默认值
不在本轮提问，用户后续可直接修改 Autopilot。
...
`resolution_enabled=true` 时默认使用高风险 P1、中风险 P2、低风险 P3；影响观测只记录一次
创建/复用基线。用户后续可直接修改 Autopilot 配置。
