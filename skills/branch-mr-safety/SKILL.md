...
All planned implementation uses one baseline, one integration branch, and two role-owned side branches:
...
- `planning_branch`: Planner-owned docs-only branch created from `source_branch`.
- `work_branch`: Builder-owned branch created from a `source_branch` that contains `approved_plan_ref`.

MRs use three stages:
...
- `planning_mr_target` must equal `source_branch`.
- `builder_mr_target` must equal `source_branch`.
- `final_mr_target` defaults to `test`.
- Planner creates only the Planning MR.
- Builder creates only the Builder MR.
- Coordinator merges the Planning MR and creates the Final MR after the required reviews and human confirmations.
...
- Planner proposes branch intent in the Ticket plan packet.
- Coordinator resolves branch fields, preserves approval evidence, and includes verified `approved_plan_ref` in Builder dispatch packets.
...
Coordinator owns Builder dispatch after planning merge and must pass the verified `approved_plan_ref`. Coordinator also owns Final MR creation after review and human confirmation.
...
6. If planning approval is required and remote `source_branch` does not exist, stop and report a blocker; Planner must create it.
7. If planning approval is explicitly not required, `source_branch_status = create_if_missing`, and remote `source_branch` does not exist, create it from latest `base_branch` and push it.
...
10. Verify `approved_plan_ref` is present when planning approval is required.
11. Verify `approved_plan_ref` is an ancestor of, and therefore reachable from, latest `source_branch`.
...
- Planning approval is required but `approved_plan_ref` is missing.
- `approved_plan_ref` is not reachable from latest `source_branch`.
...
- Planning MR target is `source_branch`.
- Planning MR changed only docs-only whitelist paths.
- Human planning approval names the exact current Planning MR head.
- Planning MR was merged by Coordinator without squash.
- `approved_plan_ref` is reachable from `source_branch`.
- `work_branch` was created from a `source_branch` containing `approved_plan_ref`.
...
- Planner: owns the planning branch, docs-only commits, and Planning MR creation. Its write whitelist is `docs/specs/**`, `CONTEXT.md`, `CONTEXT-MAP.md`, and `docs/adr/**`.
- Coordinator: owns source-branch setup, exact-head approval validation, Planning MR merge, formal issue creation, Builder dispatch with `approved_plan_ref`, reviewed Builder MR merge when policy allows, and Final MR creation after acceptance.
