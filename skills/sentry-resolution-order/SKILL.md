...
从 callback 读取 `sentry_org`、`project`、`target_assignee_type`、`target_assignee` 和 Sentry 事实快照。`source_inspection_autopilot_id` 只能是来源每日巡检 Autopilot UUID，`inspection_issue_id` 只能是本次巡检运行 Issue UUID；先执行 `multica autopilot get <source_inspection_autopilot_id> --output json`，成功取得其 `project_id` 后才允许继续。不得把 `inspection_issue_id` 当作来源 Autopilot，也不得从解决单 Autopilot 或运行根单推断来源。目标必须是用户在日报 Autopilot 中确认的 Agent 或 Squad UUID。配置缺失、类型错误、目标不存在、目标不可指派、来源 Autopilot 不可读取或无法解析时返回 `needs-info` 或 `dispatch blocked`；不得创建解决单，也不得使用 Skill 内置默认值。
