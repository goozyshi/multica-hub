...
仅通过 Sentry MCP 的 `search_issues` 查询项目分组表中每个项目。将 `source`、`time_window`、`filter`、`environment`、`sort` 和对应 `Top N` 原样应用。查询窗口必须由 Autopilot 配置的 `time_window` 明确计算：`24h` 表示运行时刻往前 24 小时，`7d` 表示往前 7 天，其他合法窗口按同一规则处理。不得改用 `search_events`、智能搜索或补造事件数。查询失败时报告对应分组和错误，不能把缺失数据当成零。
